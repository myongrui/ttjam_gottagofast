"""Single-token attention specialization.

Hypothesis: latency-bound shapes with sequence length one spend unnecessary
launches and GEMM work computing Q, K, softmax attention, and attention masks.
For a single token, softmax attention is identically the concatenated V vector,
so this specialization projects only V, skips SDPA and mask construction, and
preserves the existing output-projection-as-residual-GEMM path. Invalid tokens
remain correct because the model's final padding mask zeros their outputs.
"""
...
class FusedAttention(nn.Module):
    ...
    def forward(
        ...
        if x.shape[1] == 1:
            return F.linear(
                x,
                self.qkv.weight[2 * self.d_model:3 * self.d_model],
                self.qkv.bias[2 * self.d_model:3 * self.d_model],
            )
