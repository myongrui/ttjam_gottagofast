...
Hypothesis: padded-token zeroing after every transformer block is unnecessary for
the outputs of valid tokens, because attention already excludes padded keys and all
remaining operations are tokenwise.  This implementation removes the per-layer
masked_fill launch and applies the required zeroing only once after final LayerNorm,
eliminating one kernel launch per layer while preserving valid-token computation.
"""
import torch
