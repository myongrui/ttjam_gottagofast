"""Native multi-head-attention fast-path candidate.

Hypothesis: on unmasked non-causal inference calls, PyTorch's native fused
multi-head-attention operator can structurally replace the separate QKV
projection, SDPA invocation, attention-output projection, and intermediate
attention-layout handling.  The fast path is selected only when its semantics
exactly match the reference; causal or padded attention retains the established
SDPA implementation and residual-as-GEMM projection path.
"""
import torch
...
class FusedAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, causal=False):
        if (
            ...
            return self._native_attention(x), True
        ...
        return ctx.transpose(1, 2).reshape(b, s, self.d_model), False
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask, causal):
        ctx, already_projected = self.attention(
            self.norm1(x), attn_mask, causal
        )
