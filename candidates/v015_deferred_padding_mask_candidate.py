"""Deferred-padding-mask candidate.

Hypothesis: padded-token zeroing after every transformer block is redundant and costs
one launch per layer on masked inputs. Valid-token computations never depend on padded
query positions, while padded key/value positions are already excluded by attention's
key mask. Therefore this implementation preserves padded intermediate values through
the stack and applies the required zeroing only once after the final normalization,
eliminating the per-layer masked_fill launches without changing any valid output or
