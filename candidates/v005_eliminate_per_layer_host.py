"""v004 -- eliminate per-layer host synchronization from padded attention.

Hypothesis: v001 loses latency on the small launch-bound cases because
`bool(valid_token_mask.all())` synchronizes the CPU with CUDA four times per
forward layer stack. This candidate removes that device-to-host decision
entirely: None still selects SDPA's causal fast path, while every supplied mask
uses SDPA's device-resident boolean masking path. The mask is never inspected
on the host, so attention launches can remain queued across layers.
"""
import torch
