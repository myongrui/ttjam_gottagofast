"""Homogeneous-coordinate FFN bias candidate.

Hypothesis: encoding each FFN output bias in two constant GELU channels removes
the separate broadcast-add kernel preceding every FFN residual addmm. The two
channels carry an fp16-exact high component and a corrective low component,
preserving float32 bias accuracy while folding the bias into the GEMM itself.
"""
...
def load_from_baseline(model, baseline) -> None:
    with torch.no_grad():
        ...
        for index, src in enumerate(baseline.layers):
            ...
            ffn_dim = src.ffn_in.weight.shape[0]
            ...
            ffn_in_weight[:ffn_dim].copy_(src.ffn_in.weight)
            ffn_in_bias[:ffn_dim].copy_(src.ffn_in.bias)
            ffn_in_weight[ffn_dim:].zero_()
            ffn_in_bias[ffn_dim:].fill_(8.0)

            output_weight = state[prefix + "ffn_out.weight"]
            output_weight[:, :ffn_dim].copy_(src.ffn_out.weight)

            bias = src.ffn_out.bias
