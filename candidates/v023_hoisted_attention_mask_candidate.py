"""Hoisted-attention-mask candidate.

Hypothesis: the padded-key attention mask and its all-valid dispatch decision are
identical for every transformer layer in one forward call.  This implementation
decides that path once in Model.forward and constructs the additive attention mask
once, then reuses it across all layers.  The mechanism removes three device-to-host
synchronizations and repeated mask-construction kernel sequences per multi-layer
forward while preserving the same SDPA inputs and deferred padded-output zeroing.
"""
import torch
...
class Model(nn.Module):
    ...
    def _prepare_attention_mask(self, x, valid_token_mask):
        if valid_token_mask is None:
            return None

        # This is intentionally performed once per forward, rather than once per
        # attention layer.  The unmasked SDPA route remains the fastest route for
        # all-valid batches.
        if bool(valid_token_mask.all()):
            return None

        b, s = valid_token_mask.shape
        ...
        return attn_mask.masked_fill(~keep, float("-inf"))

    def forward(self, x, valid_token_mask=None):
        attn_mask = self._prepare_attention_mask(x, valid_token_mask)

        for layer in self.layers:
            x = layer(
                x,
                attn_mask,
                self.config.causal if attn_mask is None else False,
            )
