"""Native-MHA unmasked attention candidate.

Hypothesis: on the common non-causal, fully-valid-token path, replacing separate
QKV GEMM, SDPA, context reshape, and output-projection GEMM launches with
PyTorch's fused native multi-head-attention operator reduces launch overhead.
Causal and padded-token paths retain the established SDPA implementation so that
causal masking and padded-key semantics remain exact.
"""
import torch
...
class FusedAttention(nn.Module):
    ...
    def project_residual(self, residual, ctx, native_projected):
        if native_projected:
            return residual + ctx
        b, s, d = residual.shape
        c = residual.reshape(b * s, d) + self.out_proj.bias
        return torch.addmm(
            c,
            ctx.reshape(b * s, d),
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask, causal):
        native_projected = attn_mask is None and not causal
        ctx = self.attention(self.norm1(x), attn_mask, causal)
        x = self.attention.project_residual(x, ctx, native_projected)
