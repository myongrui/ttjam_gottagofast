"""Residual-as-GEMM-C candidate.

Hypothesis: the attention-output and FFN-output residual additions are launch-bound
on the target small-width cases.  Instead of producing a biased projection and
launching a separate residual add, this implementation precombines the projection
bias with the residual and supplies that tensor as the C operand of addmm.  cuBLAS
then performs the residual accumulation inside the projection GEMM (beta=1),
removing the post-projection residual-add dependency and making the next LayerNorm
available directly from the GEMM result.
"""
import torch
