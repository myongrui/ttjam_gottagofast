"""Short-sequence mask-dispatch candidate.

Hypothesis: on the latency-bound short-sequence cases, the device-to-host
synchronization caused by testing whether a padding mask is all-valid costs more
than using SDPA's native boolean attention-mask path.  This implementation uses
a synchronization-free boolean mask for short sequences, while retaining the
champion's float-mask/all-valid fast path for larger sequences where mask
construction and attention throughput dominate.  Padding is still deferred until
the final normalization, preserving valid-token results.
"""
import torch
...
class FusedAttention(nn.Module):
    ...
    def forward(self, x, valid_token_mask=None, causal=False):
        ...
        elif not bool(valid_token_mask.all()):
            keep = valid_token_mask[:, None, None, :]
            if causal:
                keep = keep & torch.ones(
                    (s, s), device=x.device, dtype=torch.bool
