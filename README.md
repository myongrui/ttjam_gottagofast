...
1. **A fused LayerNorm + residual Triton kernel.** The highest-value remaining
   ...
   GEMMs.
2. **A real case-14 specialist.** Chunked FlashAttention with an independently
...
3. **Close the P1 gap: choose experiments by measured gap-to-roofline.**
...
4. **Shape-generalized dispatch.** Replace the literal tuple key sets with measured
...
5. **Broaden the hardware evidence.** Re-run the full sweep on at least one other
...
6. **A hardened accuracy story for reduced precision.** Selective fp16/bf16 —
